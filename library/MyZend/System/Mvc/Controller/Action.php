...
namespace MyZend\System\Mvc\Controller;

use MyZend\System\Event\StaticEventManager;
// use MyZendTrung\Module\Model as ModuleModel;
use MyZend\SystemDb\User\Acl;
            ...
            if (!$auth->hasIdentity()) {
                ...
                if (!in_array(
                    ...
                    return $this->redirect()->toRoute(
                        'admin/login',
                        array('action'=>'index','redirect' => base64_encode($this->getRequest()->getRequestUri()))
                    );
                }
            } else {
                if (!in_array($routeName, array('admin/login','admin/forbidden'))) {
    ...
    protected function checkAcl(UserModel $userModel)
    {
        if (!empty($this->aclPage) and $userModel->getRole(true)->getName() !== RoleModel::PROTECTED_NAME) {
            ...
            if (!$acl->isAllowed(
                ...
                return $this->redirect()->toRoute('admin/not-permission');
            }
        }
    }
}
